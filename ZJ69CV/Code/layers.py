...
def AffineTransformLayer(Image, Param):
    ...
    A = tf.compat.v1.matrix_inverse(A)
    T = tf.matmul(-T, A)

    T = tf.reverse(T, (-1,))
    A = tf.compat.v1.matrix_transpose(A)
    ...
    def affine_transform(I, A, T):
        ...
        outPixelsMinMin = tf.compat.v1.to_float(tf.compat.v1.to_int32(SrcPixels))
        dxdy = SrcPixels - outPixelsMinMin
        dx = dxdy[:, 0]
        dy = dxdy[:, 1]

        outPixelsMinMin = tf.reshape(tf.compat.v1.to_int32(outPixelsMinMin),[IMGSIZE * IMGSIZE, 2])
...
def LandmarkTransformLayer(Landmark, Param, Inverse=False):
    ...
    if Inverse:
        A = tf.compat.v1.matrix_inverse(A)
        T = tf.compat.v1.matmul(-T, A)

    return tf.reshape(tf.compat.v1.matmul(Landmark, A) + T, [-1, N_LANDMARK * 2])
...
def LandmarkImageLayer(Landmarks):
    ...
    def draw_landmarks(L):
        def draw_landmarks_helper(Point):
            intLandmark = tf.compat.v1.to_int32(Point)
            locations = Offsets + intLandmark
            dxdy = Point - tf.compat.v1.to_float(intLandmark)
            offsetsSubPix = tf.compat.v1.to_float(Offsets) - dxdy
